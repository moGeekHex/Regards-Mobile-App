...
import { StyleSheet, View, ScrollView, TouchableOpacity, KeyboardAvoidingView, Platform, FlatList, ActivityIndicator } from 'react-native'
import { Card, PopularProduct, Title, ProductButton, InputPhone, InputSearch, Button, CardPhone, Input  } from '../../../components';
...
import { getProductsByEvent } from '../State/action/ProductByEventAction';
import { getProductsByCategories } from '../State/action/ProductByCategoryAction';
import { getProductsBySeller } from '../State/action/ProductBySellerAction';
...
const SearchDetails = ({ route, navigation }) => {
     ...
     const { productsBySeller, moreLoading, isListEnd } = useSelector(state=>state.productBySeller)
     const { productsByCategory, loadingProductByCategory } = useSelector(state=>state.productByCategory)
     const { productsByEvent, loadingData } = useSelector(state=>state.productByEvent)
     ...
     const handleGetProductByFilter = () => {
          ...
          {
               dispatch(getProductsByEvent(route.params?.eventID, page ));
          }
                                   ...
                                   {/* <Card pushDown="3" style={{ flexDirection: lang === "english" ? 'row' :'row-reverse', alignItems: 'center' }}>
                                        ...
                                        <View style={styles.containerPopularProducts(lang)}>

                                             {
                                                  ...
                                                  // ?
                                                  //      productsByCategoryDetails.map((productByCategory, key) => {
                                                  //           return (
                                                  //                <PopularProduct 
                                                  //                     large 
