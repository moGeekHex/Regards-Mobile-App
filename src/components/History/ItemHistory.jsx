...
const ItemHistory = ({ handleTrack, reportOrder, data, handleRepurchase, handleInvoice }) => {

     const { t, i18n } = useTranslation();
     const lang = i18n.language

     const disabledRepurchase = data.product.isActive && data.product.status === "ACCEPTED" && data.vendor.S_isBlocked === false && data.product.stockCount >= data.quantity ? false : true
     ...
     return (
          <View style={styles.root}>
               ...
               </Card>
               {
                    !reportOrder 
                    ?
                         <Card pushUp="2" flexDirection={ lang === "english" ? "row" : "row-reverse" } style={{  justifyContent : 'space-between' }}>
                              <TouchableOpacity onPress={handleTrack} style={styles.Button}>
                                   <Title 
                                        ...
                                        color="#4F008E" 
                                        text={ lang === "english" ? "TRACK" : "تتبع الطلب" }
                                   />
                              </TouchableOpacity>
                              <TouchableOpacity 
                                   style={styles.ButtonMove(disabledRepurchase)} 
                                   onPress={handleRepurchase} 
                                   disabled={disabledRepurchase}
                              >
                                   <Title 
                                        size="1.5" 
                                        fontWeight="500" 
                                        letterSpacingText={1.6} 
                                        color="#fff" 
                                        text={ lang === "english" ? "REPURCHASE" : "إعادة الشراء" }
                                   />
                              </TouchableOpacity>
                         </Card>
                    :
